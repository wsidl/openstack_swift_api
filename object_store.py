...
_OS = _OS_CONNECTION.object_store
__containers = [cont.name for cont in _OS.containers(prefix=c.SWIFT_CONTAINER)]
if not __containers:
    _OS.create_container(name=c.SWIFT_CONTAINER)
...
def add_object(content: bytes) -> c.T_RESPONSE:
    ...
    while not new_hash:
        ...
        if test_object[c.RESP_SUCCESS] and test_object[c.RESP_CONTENT]:
            ...
            continue
    try:
        _OS.upload_object(container=c.SWIFT_CONTAINER, name=new_hash, data=content)
...
def drop_object(name: str) -> c.T_RESPONSE:
    _OS.delete_object(name, container=c.SWIFT_CONTAINER)
...
def list_objects(*names: str) -> c.T_RESPONSE:
    contents = []
    names = set(names)
    for obj in _OS.objects(container=c.SWIFT_CONTAINER):
...
def get_object(name: str) -> c.T_RESPONSE:
    for obj in _OS.objects(container=c.SWIFT_CONTAINER, prefix=name):
...
def download_content(name: str) -> c.T_RESPONSE:
    content = _OS.download_object(name, c.SWIFT_CONTAINER)
